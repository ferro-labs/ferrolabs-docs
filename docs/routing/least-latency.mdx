...
- **`weight` is ignored.** `targets[].weight` has no effect under this mode — it is read by `load-balance` and, for equal-cost ties, `cost-optimized`.
