...
```yaml
strategy:
  mode: fallback  # single | fallback | load-balance | conditional | least-latency | cost-optimized | content-based | ab-test
```
...
| `load-balance` | Pool | Weighted random distribution across targets (`targets[].weight`). |
...
```yaml
strategy:
  mode: load-balance         # or ab-test
  sticky:
    on: user                 # the only supported key
    ttl: 1h                  # optional; a pin lasts at most one window
```

Under `load-balance` and `ab-test`, `sticky` pins each request to the same target — or variant — for the same `user` field, so a conversation keeps its provider prompt cache and a session does not flip variants. It is a stateless hash: no shared state, the same answer on every replica, a random draw for a request without `user`. Refused under any other mode.
...
targets:
  - virtual_key: openai
    weight: 1.0           # relative share under load-balance only; ignored elsewhere
    retry:
      attempts: 3
...
| `weight` | float64 | Relative share under `load-balance`, and the tie-break among equal-cost targets under `cost-optimized`. `0` drains the target. Negative (any mode that reads it) or all-zero under `load-balance` is a load error. |
...
A `429`, a client disconnect, a caller-supplied deadline, an unsupported-parameter rejection, and a concurrency shed do **not** count toward opening the circuit breaker. A redirect, a `5xx`, a connection failure, and the gateway's own `request_timeout` or stream idle bound elapsing all do. Inside a configured `circuit_breaker` block, omitted thresholds and timeout retain the defaults shown above; an explicitly written zero, negative, or empty value is rejected as of v1.5.6.
...
See [Plugins](/plugins) for all 11 built-in OSS plugins and their full option sets. The five content guardrails (`pii-redact`, `secret-scan`, `prompt-shield`, `schema-guard`, `regex-guard`) are documented together under [Content guardrails](/plugins/guardrails).
...
Copy [`config.example.yaml`](https://github.com/ferro-labs/ai-gateway/blob/main/config.example.yaml) from the repository for a fully annotated example covering all 30 providers, both routing families, all 11 OSS plugins with correct multi-stage registration, MCP (both transports), compatibility, and observability.

## Related pages

- [Routing](/routing) — all 8 strategies with examples
- [Plugins](/plugins) — detailed OSS plugin documentation
- [Content guardrails](/plugins/guardrails) — PII, secret, injection, regex, and response-schema policies
