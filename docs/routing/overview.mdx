...
keywords: [AI gateway routing, LLM routing strategies, fallback routing, load balancing, cost-optimized routing, least-latency routing, conditional routing, circuit breaker, model_map, failover-safe failures]
slug: /routing
---
...
| is already being avoided | circuit breaker open, concurrency queue full | next target |
...
Retry (below) re-asks the same target first for a transport failure or a retryable status; a hung attempt, an open circuit and a full queue are never retried and advance at once. Under a named mode the chosen target's result — after its own `retry` — is always the answer.

```mermaid
flowchart LR
    A[Attempt at target N fails] --> B{Client cancelled or<br/>deadline passed?}
    B -- yes --> S[Return the failure]
    B -- no --> C{Provider problem?<br/>unreachable, timed out, 408/429/5xx,<br/>circuit open, saturated}
    C -- no --> S
    C -- yes --> D{Pool mode?}
    D -- no --> S
