...
| `ferro.routing.strategy` | Emitted | Routing strategy used (`fallback`, `load-balance`, …) |
...
| `ferro.routing.attempt` | Emitted | Routing-layer attempt count when the walk ended — provider calls plus local breaker or concurrency refusals, retries and failovers included; the same number as the `X-Gateway-Attempts` response header (since v1.5.2) |
