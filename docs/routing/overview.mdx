...
| Traffic shared between providers | [`load-balance`](/routing/loadbalance) | 70 % OpenAI, 30 % Azure OpenAI; `weight: 0` drains one before you remove it. |
...
```yaml
strategy:
  mode: fallback   # single | fallback | load-balance | least-latency |
...
Mode-specific keys live under `strategy:` alongside `mode`: `conditions[]` (conditional), `content_conditions[]` (content-based), `ab_variants[]` (ab-test), `unpriced_strategy` (cost-optimized) and `sticky` (load-balance and ab-test). Each is documented on that strategy's own page. One key applies to every pool mode and rule chain:
...
| `weight` | float64 | `0` | Relative share under `load-balance`, and the tie-break among equal-cost targets under `cost-optimized`; ignored by every other mode. `0` means zero traffic — the drain lever. |
...
|---|---|---|
| **Pool** | `fallback`, `load-balance`, `least-latency`, `cost-optimized`, `ab-test` | Advances to the next candidate after a **failover-safe** failure; any other failure is returned. |
